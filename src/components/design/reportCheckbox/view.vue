    ...
    <div>
        <BaseSlot :item="props?.item">
            <a-form-item :label="props?.item?.data?.label" :rules="[{ required: props?.item?.required, message: `Please enter ${props?.item?.title}`, trigger: 'change' }]">
                <a-checkbox-group v-model:value="props.value.value" :options="props?.item?.data?.options" v-if="props?.item?.layout == 'horizontal'" />
                <a-checkbox-group v-else v-model:value="props.value.value">
            ...
            <div v-if="props?.item?.hasResult" style="margin-top:10px;">
                <a-input v-model:value="props.value.result" :placeholder="$t('base.PleaseEnterResult')" />
            </div>
            <div v-if="props?.item?.hasRemark" style="margin-top:10px;">
                <a-textarea v-model:value="props.value.remark" :placeholder="$t('base.PleaseEnterRemark')"/>
            </div>
        </BaseSlot>
    </div>
</template>
