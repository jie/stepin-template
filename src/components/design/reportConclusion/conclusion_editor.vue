  ...
  <div>
    ...
    <div style="padding-left: 80px">
      <div style="margin-top:10px;">
        ...
        <a-form-item :label="$t('base.ParentConclusionItem')" name="parent_key"
    ...
    <a-modal :getContainer="() => document.body" v-model:visible="isShowOptionDialogRef" :title="$t('base.EditOption')"
      @ok="handleOptionOK">
...
const switchLanguageType = () => {
  if (itemData.languageType === 'single') {
    itemData.languageType = 'multiple'
    // itemData.options = newJsonObject(multipleLanguageOptions)
  } else {
    itemData.languageType = 'single'
    // itemData.options = newJsonObject(singleLanguageOptions)
  }
}

const onChangeParentComKey = () => {
  conclustionComponentRecords.value = reportTemplateStore.reportTemplate.items.find(c => c.key === itemData.parent_com_key).data.conclusions
}
...
const onClickAction = (type: string, text: string, index) => {
  ...
  if (type == 'EditOutlined') {
    ...
    Modal.confirm({
      ...
      onOk() {
        console.log('OK')
        itemData.conclusions.splice(index, 1)
...
const onClickEditConclusion = () => {
  ...
  isShowConclusionDialogRef.value = true
  const conclusion = itemData.conclusions[currentEditConclusionIndexRef.value]
...
const onClickOption = (index: number) => {
  ...
  optionFormData.label = itemData.options[index].label
  optionFormData.value = itemData.options[index].value
}

const handleOptionOK = () => {
  itemData.options[currentEditOptionIndexRef.value] = newJsonObject(optionFormData)
...
const handleConclusionOK = () => {
  if (isEditConclusionRef.value === true) {
    itemData.conclusions[currentEditConclusionIndexRef.value] = newJsonObject(conclusionFormData)
  } else {
    if (currentEditConclusionIndexRef.value !== null) {
      itemData.conclusions.splice(currentEditConclusionIndexRef.value + 1, 0, newJsonObject(conclusionFormData))
    } else {
      itemData.conclusions.push(newJsonObject(conclusionFormData))
    }
  }
  currentEditConclusionIndexRef.value = null
...
const initializeData = (item: any) => {
  ...
  if (item?.data) {
    let newItemData = newJsonObject(item.data)
    itemData.parent_com_key = newItemData.parent_com_key || ''
    itemData.parent_key = newItemData.parent_key || ''
    itemData.languageType = newItemData.languageType || 'single'
    itemData.conclusions = newItemData.conclusions || []
    itemData.options = newItemData.options || []
  } else {
    itemData.parent_com_key = ''
    itemData.parent_key = ''
    itemData.languageType = 'single'
    itemData.conclusions = []
  }
