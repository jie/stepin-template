...
const newReportCompany = (reportCompany?: ReportCompany) => {
...
const form = reactive<ReportCompany>(newReportCompany());
console.log('form:', form)
function reset() {
  return newReportCompany(form);
}
...
const editRecord = ref<ReportCompany>();

/**
 * 编辑
 * @param record
 */
function edit(record: ReportCompany) {
  isViewForm.value = false
...
function view(record: ReportCompany) {
  createTimeRef.value = dayjs(record.create_at)
  Ctl.isNew = false;
  editRecord.value = record;
  copyObject(form, record);
...
const goDesign = (record: ReportCompany) => {
  router.push({
    name: 'design',
    query: {
      id: record.id,
        ...
        <a-dropdown>
          ...
          <template #overlay>
            <a-menu>
              ...
              <a-menu-item key="1">
                <a-popconfirm title="Delete" :content="$t('base.ConfirmDelete')" :okText="$t('base.Yes')" :cancelText="$('base.No')"
